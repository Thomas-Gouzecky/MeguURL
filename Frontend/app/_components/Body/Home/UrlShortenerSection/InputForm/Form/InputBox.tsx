...
export default function InputBox({ status }: { status: StatusProp }) {
	...
	return (
		<input
			className={`${statusStyle[status.state]} disabled:bg-amber-50 flex-1 z-10 origin-center bg-[#130b0b] outline-none focus:scale-100 hover:shadow-[0_0_10px_3px_rgba(0,0,0,0.25)] border-2 button-padding button-rounding relative text-lg font-bold overflow-hidden transition-all duration-300`}
			name="inputUrl"
			type="text"
